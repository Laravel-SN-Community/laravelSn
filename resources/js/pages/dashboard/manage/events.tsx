...
function statusStyle(status: string): { label: string; bg: string; color: string } {
    if (status === 'published') {
        return { label: 'Publié', bg: 'color-mix(in oklch, var(--sn-600) 12%, transparent)', color: 'var(--sn-700)' };
    }

    if (status === 'draft') {
        return { label: 'Brouillon', bg: 'color-mix(in oklch, #f59e0b 12%, transparent)', color: '#b45309' };
    }

    return { label: status, bg: 'var(--sn-surface-2)', color: 'var(--sn-muted)' };
}

export default function ManageEvents({ events, stats, venues, filters }: Props) {
...
setEditTarget(null);
    ...
    function go(newTab: string) {
        router.get(EventController.manageIndex.url(), { tab: newTab }, { preserveState: true, replace: true });
    }
...
return;
        ...
        router.delete(EventController.destroy.url({ event: deleteTarget.slug }), {
    ...
    return (
        <>
                ...
                <div className="grid gap-8 lg:grid-cols-[240px_1fr]">
                    ...
                    <main className="min-w-0 space-y-6">
                        ...
                        <div className="flex flex-wrap items-end justify-between gap-3">
                            ...
                            <button onClick={openCreate} className="sn-btn sn-btn-primary shrink-0">
                                <Plus size={15} />
                                Nouvel évènement
                            </button>
                        </div>
                            ...
                            {TABS.map((t) => {
                                ...
                                return (
                                    <button
                                        ...
                                        style={{
                                            background: active ? 'var(--sn-700)' : 'transparent',
                                            color: active ? '#fff' : 'var(--sn-muted)',
                        ...
                        >
                            {events.data.length === 0 ? (
                                <div className="px-6 py-12 text-center font-mono text-[13px]" style={{ color: 'var(--sn-muted)' }}>
                        ...
                        {events.last_page > 1 && (
                            <div className="flex justify-center gap-2 font-mono text-[12px]">
                                {events.prev_page_url && (
                                    <Link href={events.prev_page_url} className="sn-btn sn-btn-ghost sn-btn-sm">← Précédent</Link>
                                )}
                                <span className="flex items-center px-3" style={{ color: 'var(--sn-muted)' }}>
                                    {events.current_page} / {events.last_page}
                                </span>
                                {events.next_page_url && (
                                    <Link href={events.next_page_url} className="sn-btn sn-btn-ghost sn-btn-sm">Suivant →</Link>
                                )}
                            </div>
                        )}
                    </main>
            ...
            {/* Delete confirm */}
            <Dialog.Root open={deleteTarget !== null} onOpenChange={(open) => {
 if (!open) {
setDeleteTarget(null);
                ...
                <Dialog.Portal>
                    ...
                    >
                        ...
                        <Dialog.Title className="mt-4 text-[17px] font-semibold tracking-tight" style={{ color: 'var(--sn-fg)' }}>
                            Supprimer l'évènement
                        </Dialog.Title>
                        <Dialog.Description className="mt-1.5 text-[13.5px] leading-relaxed" style={{ color: 'var(--sn-muted)' }}>
                            <span className="font-medium" style={{ color: 'var(--sn-fg)' }}>«&nbsp;{deleteTarget?.title}&nbsp;»</span>{' '}
                            sera supprimé définitivement. Cette action est irréversible.
                        </Dialog.Description>
                        <div className="mt-6 flex justify-end gap-3">
                            <Dialog.Close className="sn-btn sn-btn-secondary sn-btn-sm">Annuler</Dialog.Close>
                            <button
...
function EventRow({ event: e, last, onEdit, onDelete }: { event: ManageEvent; last: boolean; onEdit: () => void; onDelete: () => void }) {
    ...
    const meta = (
        <div className="flex flex-wrap items-center gap-1 font-mono text-[10px] tracking-[0.15em] uppercase" style={{ color: 'var(--sn-muted)' }}>
            <span>{FORMAT_LABELS[e.format] ?? e.format}</span>
            {e.venue && <><span>·</span><span>{e.venue.district}</span></>}
            {e.is_online && <><span>·</span><span>En ligne</span></>}
            {e.is_featured && <><span>·</span><span style={{ color: 'var(--sn-600)' }}>★ Une</span></>}
        </div>
    );
    ...
    const actions = (
        <div className="flex items-center gap-1.5">
            ...
            <button
                ...
                style={{ background: 'color-mix(in oklch, var(--destructive) 8%, transparent)', color: 'var(--destructive)' }}
            >
                <Trash2 size={13} />
            </button>
        </div>
    ...
    return (
        <div style={{ borderBottom: last ? 'none' : '1px solid var(--sn-border)' }}>
            {/* ── Mobile layout ── */}
            <div className="space-y-2 px-4 py-4 sm:hidden">
                ...
                <div className="text-[15px] font-semibold leading-snug tracking-tight" style={{ color: 'var(--sn-fg)' }}>
                    {e.title}
                </div>
                <div className="flex items-center gap-2 font-mono text-[11px]" style={{ color: 'var(--sn-muted)' }}>
            ...
            <div className="hidden items-start gap-5 px-6 py-5 sm:flex">
                ...
                <div className="w-12 shrink-0 text-center">
                    <div className="font-mono text-[10px] tracking-[0.18em] uppercase" style={{ color: 'var(--sn-muted)' }}>{monthUp}</div>
                    <div className="mt-0.5 text-[30px] leading-none font-semibold tracking-tight" style={{ color: 'var(--sn-700)' }}>{day}</div>
                    <div className="mt-0.5 font-mono text-[10px]" style={{ color: 'var(--sn-muted)' }}>{time}</div>
                </div>

                {/* Content */}
                <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 font-mono text-[10.5px] tracking-[0.15em] uppercase" style={{ color: 'var(--sn-muted)' }}>
                        <span>{FORMAT_LABELS[e.format] ?? e.format}</span>
                        {e.venue && <><span>·</span><span>{e.venue.district}</span></>}
                        {e.is_online && <><span>·</span><span>En ligne</span></>}
                        {e.is_featured && <><span>·</span><span style={{ color: 'var(--sn-600)' }}>★ À la une</span></>}
                    </div>
                    <div className="mt-1 text-[15px] font-semibold leading-snug tracking-tight" style={{ color: 'var(--sn-fg)' }}>{e.title}</div>
                    {e.venue && (
                        <div className="mt-0.5 text-[12.5px]" style={{ color: 'var(--sn-muted)' }}>
